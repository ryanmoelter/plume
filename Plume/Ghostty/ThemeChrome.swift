...
/// The sidebar's three candidate backgrounds, kept side by side so they can
/// be compared live rather than argued about from memory.
enum SidebarBackgroundStyle: String, CaseIterable, Identifiable, Sendable {
    /// The existing 50% theme tint over the sidebar `List`'s own material.
    case tint
    /// A Liquid Glass surface over the sidebar, tinted like the composer's.
    case tintedGlass
    /// The same glass surface, untinted.
    case glass
    ...
    var label: String {
        switch self {
        case .tint: "Tint"
        case .tintedGlass: "Tinted Glass"
        case .glass: "Plain Glass"
        }
    }
}
...
private struct SidebarBackgroundModifier: ViewModifier {
    let colorScheme: ColorScheme
    let style: SidebarBackgroundStyle

    func body(content: Content) -> some View {
        switch style {
        case .tint:
            content.themeTint(colorScheme: colorScheme)
        case .tintedGlass:
            glassBody(content, tint: ThemeChrome.background(for: colorScheme)?.opacity(0.5))
        case .glass:
            glassBody(content, tint: nil)
        }
    }

    private func glassBody(_ content: Content, tint: Color?) -> some View {
        let foreground = ThemeChrome.foreground(for: colorScheme)
        return content
            .scrollContentBackground(.hidden)
            .background {
                let glass = tint.map { Glass.regular.tint($0) } ?? Glass.regular
                Rectangle().glassEffect(glass, in: .rect)
            }
            .foregroundStyle(foreground == nil ? AnyShapeStyle(.primary) : AnyShapeStyle(foreground ?? .primary))
    }
}

extension View {
    /// Applies `style` as the sidebar's background. Unlike `themeTint`, this
    /// covers the glass variants Ryan is comparing live; every other
    /// `themeTint` call site is unaffected.
    func sidebarBackground(colorScheme: ColorScheme, style: SidebarBackgroundStyle) -> some View {
        modifier(SidebarBackgroundModifier(colorScheme: colorScheme, style: style))
    }
}
