...
Sparkle checks for updates in the background, and **an available update never interrupts** — no launch dialog, no system notification, nothing modal. It shows only as an "Update Available" row in the sidebar footer. Clicking it opens Sparkle's window on a DMG install, or a popover with `brew upgrade --cask ryanmoelter/tap/plume` on a Homebrew install (detected via a `Caskroom/plume` directory; Settings can override). `UpdateController` (`Plume/App/Updates/`) owns this, and Sparkle's scheduled checks use gentle reminders so Sparkle itself never shows UI unprompted. A Debug build doesn't start the updater unless the `PlumeUpdateFeedURLOverride` user default is set.
