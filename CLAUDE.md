...
- **Never `scrollTo` into the chat's `LazyVStack`.** An animated programmatic scroll into a lazy stack whose rows are still estimates never settles: each placement pass moves the target, the stack's prefetch requests another pass, and the main thread pins with the window frozen. The list follows new content through `defaultScrollAnchor(.bottom, for: .sizeChanges)` and moves through `ScrollPosition` only. **`docs/chat-list-hang.md`** has the sampled evidence and the DEBUG harness that reproduces it.
