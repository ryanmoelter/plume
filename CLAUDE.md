...
Terminals come from the `GhosttyTerminal` product of `Lakr233/libghostty-spm`, pinned `.exact("1.5.2")`. **`docs/ghostty-pin.md` is the reference** — pin details, why the wrapper was adopted, config search order, upgrade steps, and the self-vendoring fallback. Read it before touching anything Ghostty-related or upgrading the package.
...
Sparkle checks for updates in the background, and **an available update never interrupts** — no launch dialog, no system notification, nothing modal. It shows only as an "Update Available" row in the sidebar footer. Clicking it opens Sparkle's window on a DMG install, or Plume's own update window on a Homebrew install (detected via a `Caskroom/plume` directory; Settings can override). `UpdateController` (`Plume/App/Updates/`) owns this. **Plume schedules the background checks itself, with `checkForUpdateInformation()`; Sparkle's own automatic checks stay off.** A Sparkle-scheduled check holds the first update it found until relaunch, so the row would never show a newer release. A Debug build doesn't start the updater unless the `PlumeUpdateFeedURLOverride` user default is set.
...
- **Work that outlives a turn is read from the transcript, never from hooks.** A `Monitor`, a backgrounded `Bash` and a `Workflow` all keep running after the tab drops to `awaitingReply`. `BackgroundTaskScanner` finds them in the same parse `TranscriptStore` already runs, and `BackgroundTaskTracker` holds them per tab so `KeepAwakeCoordinator` can keep the Mac awake. Entries are reconciled wholesale per read, and capped at 30 minutes — a transcript that stops being written is the one thing that can never clear them. An entry retires when the transcript says its task ended, rather than waiting out the window the task declared: a terminal `<task-notification>`, or for a Monitor the call passed `persistent: false`, that monitor's first event. Only an explicit `false` counts — reading an absent `persistent` as one-shot would retire a monitor still watching and let the Mac sleep mid-work. A background `Agent` is deliberately not tracked here: `setSubagentActivity` already covers it.
...
- **The chat list places one item per *block*, not per message.** `ChatPieceSplitter` builds them and `ChatPieceView` draws them; a message's wash is painted per piece on the edges that piece owns. This dates from a SwiftUI `LazyVStack` engine that estimated unrealized items from realized ones and hung when a realized set's heights differed by a large factor; that engine is gone, replaced by Plume's own `NSScrollView`-based list, but the split itself stays because it also keeps one measured row from towering over its neighbors. **`docs/chat-list.md`** has the history, the evidence, and the measured ratios.
...
- **The chat list (`Plume/UI/Chat/List/`) is Plume's own `NSScrollView`-based container**, not SwiftUI's `LazyVStack` — it owns layout itself, in AppKit, and every visible row is measured for real before it is placed. `docs/chat-list.md` is the reference. A fresh `NSHostingView` root has none of the SwiftUI environment a view mounted normally would inherit, so any environment key a chat row reads has to be forwarded by hand in `ChatListItemRoot` or the row silently renders with defaults instead of failing loudly.
