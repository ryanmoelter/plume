...
- **`.symbolEffect` with a repeating option is not free in-process.** RenderBox re-rasterizes the symbol every frame on the main thread and blocks on the render-server commit, about a tenth of a core per instance, and the stall starves wheel-event handling. `WorkingEllipsis` steps text glyphs off a `TimelineView` for this reason; measure any repeating symbol effect in a bare window before shipping one. `docs/handoff-idle-cpu.md` has the numbers.
