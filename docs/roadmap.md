...
- [x] Make the statusline capture installable with confidence — it no longer touches the statusline in terminals outside Plume.
...
- **The statusline install is safe to recommend.** `StatuslineInstaller.install()` backs up the existing `statusLine`, writes a generated capture script, and points `~/.claude/settings.json` at it; the script writes the payload to `$PLUME_EVENTS_DIR/$PLUME_TASK_ID/$PLUME_TAB_ID.statusline.json` and then chains to the previous command. The three problems that held it back are fixed. **The capture is conditional; the chain is not** — the script tests the three `PLUME_*` variables and skips straight to the chain when any is unset or empty, so in a terminal Plume didn't launch it captures nothing and hands the user their own statusline unchanged. It used to abort there with `PLUME_EVENTS_DIR: unbound variable` under `set -u`, printing nothing. **One script path serves both builds**: `AppPaths.sharedApplicationSupport` is always `Plume`, never `Plume.debug`, and only `statuslineScriptFile` derives from it — a deliberate hole in the debug/release split, because `statusLine` is a single global value and a per-build path guarantees the two installs overwrite each other. The store, events directory and hook `settings.json` stay split as before. **The enable toggle is gone**: installed state is the switch, `isAlreadyInstalled` reads it from disk, and Settings now shows either the JSON preview with Install or the installed state with Restore and what Restore will put back.
- **It is scaffolding, not a foundation.** A statusline is TUI chrome, so the capture does nothing under headless `claude -p` — verified against Claude Code 2.1.258, where a `-p` run never executes the configured script. Headless supplies the same numbers natively instead: a `rate_limit_event` carrying five-hour and seven-day utilization, and a `result` event carrying `total_cost_usd`. So this is safe to install and not worth building further on; a transport cutover deletes it rather than porting it, and must call `StatuslineInstaller.restore()` on the way out so the user's global `settings.json` stops pointing at a script Plume no longer writes. `docs/agent-transport.md` has the field-by-field mapping.
- Installing is otherwise well-behaved and worth keeping: it's explicit (a button, never automatic), it previews the exact JSON first, it records "there was no `statusLine`" distinctly from "there was one" so restore can remove the key rather than guess, and re-installing is a no-op instead of chaining the script to itself. `sh -c` expands a leading `~`, so a chain target like `~/.scripts/.claude/statusline.sh` resolves correctly.
