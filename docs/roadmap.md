...
**Spellcheck shipped as one line.** `ScrollableComposerTextView.setUp` now sets `isContinuousSpellCheckingEnabled`, so misspellings are underlined rather than only silently autocorrected. It does not fight `MarkdownComposerStyler`: the checker's marks are layout-manager temporary attributes, and the styler's per-keystroke pass writes text-storage attributes, so neither carries the other away.
