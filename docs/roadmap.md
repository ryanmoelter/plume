...
The model list needed no new source. `AgentModel` is a static three-case enum (`fable`/`opus`/`sonnet`) that the post-launch control already enumerated with `allCases` — it never came from the `initialize` reply — so the same list serves both sides of the launch, and no hardcoded fallback was needed.
