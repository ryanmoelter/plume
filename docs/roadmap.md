...
**The bottom chrome is one floating panel, content width.** The statusline and composer surface no longer spans the pane edge to edge: it takes the content column — the same measure the prose above it wraps at — rounds all four corners to `panelCornerRadius`, and leaves `panelInset` below itself, so it reads as a panel resting over the chat rather than a bar bolted to it. There is exactly one drawn surface: the composer, a divider, and the statusline sit on the same glass, with no box of the composer's own nested inside it. Every other measurement derives from `panelCornerRadius` through `ComposerPanelMetrics`, which is why the panel's inset, the floating popups' corner radius and the dock bar's tuck are one decision rather than three literals. It all stays inside the existing `GlassEffectContainer`, which is what keeps the dock's shadow off the surface below it.
