...
- **Done is `stop_reason: end_turn`, and nothing else.** Neither of the softer signals survives contact with real transcripts. Trailing assistant prose looked like a report but is usually an agent narrating between tool calls: 49 of 210 sampled subagents ended that way while still working, every one a false green check. A `tool_result` on the spawning call is no better, because 88% of spawns (272 of 308) are async and answered immediately with "Async agent launched successfully" — and the parent's `task_status` completion attachment, the one record that would close the loop, appeared for only 2 of 241 async launches. So `SubagentStatusDeriver` keys on the **last** `stop_reason` in the subagent's own file (`Transcript.lastStopReason`, latest wins so a resumed agent reads as working again), and falls back to a non-launch parent result only for the few transcripts with no closed turn at all. Background agents have no unambiguous completion signal, and this deliberately shows them working rather than guessing done.
...
- **A finished subagent lingers 30s before folding away.** `SubagentCompletionTracker` times the move from the moment Plume *observes* the status, never from the transcript's mtime — dating them by their own timestamps would collapse every row the instant a relaunch re-read old files. It is in-memory only, and `SubagentListView` observes it from `.onChange` rather than `body`, keyed on a status signature so transcript growth alone does not restart a linger.
...
The model list needed no new source, and there is none to be had: the `init` event reports only the model in use, and `capabilities` names protocol features rather than models. So `AgentModel` carries a hand-maintained list of presets — Fable, Opus and Sonnet on the top level, the rest under a "More" submenu — plus an "Other…" field for an ID this build has never heard of. It is a struct over a CLI model ID rather than an enum, because the set is open.
