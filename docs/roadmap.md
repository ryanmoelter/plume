...
- [ ] Give each control an icon: `brain` for model, the `gauge.with.dots.needle.0percent` family for effort, and one per permission mode — `bolt.fill` for auto, `text.page` for plan, `exclamationmark.triangle.fill` for bypass, `info.circle` for the ask-every-time mode.
