...
- [ ] Label the smaller-window models 200K. `AgentModel.more` calls them "Opus 256K", "Sonnet 256K" and "Haiku 4.5 256K" (`ModelEffortCommand.swift:56-58`), but the window they actually report is 200,000 — which `nominalContextWindow` already returns for exactly these IDs, so the menu contradicts the meter beside it. The doc comments around them say 256K too and want the same correction.
- [ ] Work out why the branch segment says "no upstream" for a branch that has one.
...
**"no upstream" shows for a branch that tracks one.** Seen live on 2026-09-05: this repository's `main` reads "no upstream" although `origin/main` exists and `git status --porcelain=v2 --branch` reports both `# branch.upstream origin/main` and `# branch.ab +1 -0`. Reaching that label needs a real `GitState` whose `ahead` is nil (`GitState.hasUpstream` is `ahead != nil`), so `git` ran and its `# branch.ab` line was missing or unparsed — a failed run yields nil and draws no segment at all.

Two things to check first, both about *which* directory was measured rather than about the parse:

- **The branch name and the upstream fact come from different sources.** `ChatTabView` passes `branch: transcript.gitBranch` — what Claude Code recorded in the transcript — beside `gitState: GitStateStore.shared.state(for: gitDirectory)`, which is `git` run in the tab's working directory (`ChatTabView.swift:294-295`). When the two disagree, the segment names one branch and reports another's upstream. A worktree branch that genuinely has none is the obvious way to produce "main … no upstream".
- **A linked worktree's `.git` is a file, not a directory.** `GitStateStore.startWatching` watches `<root>/.git` and only checks that it exists, so in a worktree it watches a 69-byte pointer file that never changes; refs live in the common directory. Nothing there fails loudly — the 15s poll still refreshes — but it means a worktree's state is only ever as fresh as the poll, where the main checkout updates on every ref write.
...
`ChatBlockSpacing` now decides every vertical gap in the chat from the kind of item above it: consecutive tool calls take `Dimensions.toolCallSpacing` (4), anything else takes `messageBlockSpacing` (8) within a message or `messageSpacing` (40) between two rows. Both gaps that made a run of calls read as separate statements are covered. Within a message, `assistantBody` stacks at spacing 0 and each block pays its own top inset — counting the previous *rendered* block, so a call the pending dock draws instead does not leave a gap behind. Between rows, `ChatMessageList` classifies a message as tool calls alone and drops the gap to the same 4pt, and the rows themselves carry no vertical inset at all: every gap between two rows is the following row's top inset, which is the only shape in which one number can own it.

`StreamingBlocks` pays its own top inset rather than each mount site duplicating a constant, conditional on whether anything precedes it. Standalone in `ChatMessageList` nothing does, so it pays nothing and takes the row inset an assistant message would have taken. Inside `assistantBody` it takes the gap it will have once it settles: after prose the text stays in the same message, so it takes the block gap; after a tool call the tool's result ends the assistant's run in the transcript, so the text retires into a message of its own and already sits a message apart. That is what stops the reply shifting as the stream retires — and what fixes the streamed text reading tight against the tool call above it.
...
- [ ] Focus the composer when a new tab or task opens. The wiring is in place and does not take effect — ⌘N leaves focus elsewhere.
...
- **Composer focus is wired and does not work.** Seen live on 2026-09-05: ⌘N opens a new task and leaves the composer unfocused. The wiring reads correct, which is what makes this worth writing down. All three input views set focus from `.onChange(of: isVisible, initial: true)` — `ChatComposer` and `AgentFirstMessageView` set their `@FocusState`, `TerminalTabView` calls `session.state.requestFocus()`. `initial: true` is load-bearing: `TabVisibility.isOnScreen` is `tabID == selectedTabID`, every creation path selects the new tab synchronously (`TaskStore.createTask` / `addTab` both call `selectTab`), and `HeadlessAgentTabContent` builds `ChatTabView` only while visible — so a new tab's composer is born already visible and never sees a false→true edge for a plain `.onChange` to catch. The untested hypothesis is that focus is set and then taken away: ⌘N also assigns the sidebar's `selection` (`MainWindow.swift:47-58`), so the sidebar list may claim the window's first responder after the `@FocusState` write lands. Which other entry points fail is unconfirmed — ⌘T, the tab strip's + button and the sidebar's New Task buttons all share the same wiring and have not been checked.
