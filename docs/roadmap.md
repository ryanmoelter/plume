...
Not queued because they are not yet specified: two **requests** from a conversation outside this repo — an accurate chat status with elapsed working time, and tracking what the agent is currently doing. They are recorded with what the code already provides, but each needs a clarifying pass before it can be sized (see [Requests](#requests)). The third, a conversation minimap, shipped.
...
- [ ] Move `SubagentTranscriptOverlay` onto a `LazyVStack`, like the main chat. It renders through the same `ChatPieceView` pipeline but puts the whole transcript in a plain `VStack` inside a `ScrollView`, so every row is realized at once. Its header justifies that as "tens of rows, not thousands", which a long-running subagent does not obviously honour — and it is the one chat surface that never got the piece-splitting `docs/chat-list-hang.md` prescribes, so the migration has to bring `ChatPieceSplitter` with it rather than wrapping the existing rows.
...
**Both list suspects were real, and both are fixed.** `MarkdownBlock.parse` now scans a list through blank lines between items and through a wrapped item's continuation line, which it joins onto the item with a space; a fence, heading, table, rule, quote, the other list kind, or prose after a blank line still ends it. The source's first number survives as `numberedList(_, start:)` and reaches `ListSegment.startNumber`, so a list starting at 3 counts 3, 4, 5 — later numbers are still discarded, so a source that repeats `1.` renders 1, 2, 3.

A bash row's input and result no longer render alike: the input is titled "Command" and keeps the filled code block, while the result is titled "Output" and is outlined and secondary instead of filled. `ToolCallInputRendering` already handed Bash its raw multi-line `command`, so the newlines were real there; what was escaped onto one line was the JSON of every other tool, and `TranscriptParser.prettyPrint` now actually pretty-prints.

A heading's inline code takes the heading's size: `MarkdownBlockView.headingStyle(level:)` supplies both the `Text` font and the size handed to `MarkdownCache.styledInline`. The size already keyed the cache, so nothing new invalidates.
...
- [x] Autocomplete slash commands in the first message. A tab with no session yet offers none at all, so the one message most likely to be a `/` command is the one with no help typing it.
...
- [x] Spellcheck the composer, the way every other Mac text field does.
- [x] Remember the autocompleted commands per directory. One global list offered a project's own commands in every other project.

**Spellcheck marks, it never rewrites.** `ScrollableComposerTextView.setUp` sets `isContinuousSpellCheckingEnabled` and turns every rewriting setting off: automatic spelling correction, quote and dash substitution, text replacement, link and data detection, and grammar checking. Automatic correction shipped on by mistake and silently replaced identifiers and paths as they were typed. It does not fight `MarkdownComposerStyler`: the checker's marks are layout-manager temporary attributes, and the styler's per-keystroke pass writes text-storage attributes, so neither carries the other away.

**Code is exempt from checking.** `ComposerCodeRanges` reads `MarkdownHighlighter`'s `.inlineCode` and `.codeBlock` spans, so "is this code?" has one parser rather than two, and the composer's coordinator answers three `NSTextViewDelegate` questions from it: `willCheckTextIn` drops the spelling and grammar types for a range that is entirely code, `didCheckTextIn` discards the spelling and grammar results that land in code, and `shouldSetSpellingState` returns 0 for a range touching code. Text checking picks its own ranges, which straddle span boundaries, so the two result-level hooks are what actually carry the behavior — `willCheckTextIn` only helps when a whole checked range is code. `shouldSetSpellingState` is the backstop for whichever path continuous checking takes.
...
**The first message autocompletes from the last session's list in that directory.** `SlashCommandMemory` holds what the CLI last reported, keyed by directory and persisted to `UserDefaults`, and `ChatComposer.availableSlashCommands` falls back to it whenever the live session has yet to answer `initialize` — a cold tab included. Keying by directory is what keeps one project's project-scoped commands out of another: the `commands` array does not mark which entries are project-scoped, so there is nothing to filter on after the fact, and each directory simply remembers what was reported there. `TabDirectoryStore` supplies the key on both sides — where the tab is now, else where its process started. The list holds the twenty most recently reported directories, so it cannot grow an entry per worktree forever. `HeadlessSession.applyReportedCommands` rewrites the memory on every `initialize` rather than caching once, so adding or removing a project command corrects the list. An `initialize` reply carrying no `commands` at all leaves the memory alone; it says nothing about the installation. While the names on offer are the remembered ones, the popup captions itself "From the last session — not yet confirmed by Claude Code", following `hasReportedModeAndModel`'s precedent.

`/rc` is listed only once a session exists, because that is also the only time it can be *run*: it drives a `remote_control` control request, which needs a live process to reach. Listing Plume's own command in a cold tab would offer a completion that does nothing on send.
...
- [x] Make Return in the feedback field send the feedback. It approves the plan instead, which is the opposite of what the field invites.
- [x] Give the feedback field the composer's text: the same font, and the same basic markdown styling as you type.
- [ ] Confirm multi-line feedback and the send-key setting actually work, once Return reaches the field at all.
- [x] Keep an undecided plan reachable: while a proposal is awaiting a decision, minimize is the only way out of the overlay. Hide the close button and drop its ⎋ shortcut, and hide the dock bar's close button too, so the plan can never leave the screen entirely before it is approved or rejected.
...
- [x] Name the plan in the minimized dock bar. It showed the file name alone, which says which file holds the plan but not what the plan is.
...
**The dock bar says what the plan is, not only which file holds it.** `ChatTabView` owns the `MarkdownFileStore` and follows the plan file whenever one exists, rather than only while the overlay is up — the dock bar is what shows when it is not, and one store means one read. `PlanSummary.title(of:)` takes the plan's first heading and the bar puts it ahead of the file name, which stays beside it in secondary emphasis at the same size, so the bar is exactly as tall with a title as without and the conversation above it never shifts. A plan file exists before it is ever proposed, so a file with no heading yet is ordinary: the bar falls back to the file name alone. The expanded panel's header keeps the file name by itself, since the plan's own `#` heading renders two rows below it. `MarkdownFileView` became `MarkdownContentView`, taking content rather than a path, so the caller that already needs the text does the reading.

**An undecided plan is no longer closable.** `PlanApprovalState.isClosable` gates both close buttons, and the ⎋ shortcut goes with the panel header's, so while a proposal awaits a decision minimize is the only exit. Closing returns once the plan is approved or rejected, where the overlay is just a viewer again.
...
**The feedback field is now the composer's editor, and Approve is no longer the default button.** `planApprovalOptions` hosts `MarkdownComposerTextView`, so a note gets the prose face and live markdown styling wherever it is typed, and Return follows `composerSendKey` through the one rule in `ComposerNSTextView.keyDown`. `PlanFeedbackKey` is gone with it — the rule lives in one place rather than being restated for a `TextField`.

`.defaultAction` on Approve was the thing taking Return, and swapping the editor in would not have been enough on its own: AppKit runs `performKeyEquivalent` down the view tree before delivering a key to the first responder, so a default button answers Return ahead of any focused text view. That shortcut is removed; Approve keeps `.borderedProminent` for its prominence. The third option keeps its key through a new `onOptionReturn` hook on the editor, which the composer itself leaves nil.
...
- [x] Extend the height easing to every row, including the turn in flight and a row whose wash joins its neighbours.
- [x] Grow a newly arrived row into place, so a tool call landing pushes the conversation up instead of teleporting it.
- [x] Ease the room the floating composer covers, so a composer gaining a line slides the messages.
- [x] Keep the reveal running across a paragraph boundary, finishing the block that completed before starting the one that opened.

What shipped:

- `AnimatedHeight` (`Plume/UI/Chat/`) measures an item's ideal height with `onGeometryChange` inside `fixedSize`, then drives an explicit `frame(height:)` from it. Measuring inside `fixedSize` is what stops the animated frame feeding back into its own input. Later measurements ease at `.easeOut(duration: 0.2)`. The first is assigned without animating unless the caller passes an `initialHeight` to grow from, which is how an arriving row starts at zero; growing rows also fade and are clipped, so the content opens with the box instead of being legible before it has room.
...
- **`ChatPieceView` owns the frame, inside the wash and outside the vertical padding it pays.** The wash sizes to the animated frame, so a joined segment cannot open a seam, and neither the padding nor the segment a piece turns out to be — both change when a neighbour does — jumps on its own.
- **The item carrying the live stream eases too**, at a shorter `.easeOut(duration: 0.12)`, because its height steps at every line wrap while the reveal draws and a longer ease trails the text. This reverses an earlier decision, and the harness numbers below are the reason to keep watching it: animating the streaming row alongside the reveal was the one configuration that tripped `detect-chat-hang.sh`, at t=221s. That was read as load rather than the recorded loop — the sampled frames were plain layout work with no `signalPrefetch → requestUpdate` — and the later trials falsified animation as a participant in the loop outright. Re-run that harness configuration before believing this is free.
- **`ChatListMotion` names the pieces that arrived**, comparing each rebuild's ids against the last. It names none on the first build, and none on a rebuild carrying a changed streaming overlay — the overlay settles a piece every time a paragraph finishes and swaps all of them for the transcript's own at the end of a turn, and growing those would fight the reveal.
- **The `ForEach` diff is never wrapped in an animated transaction, and no row carries a `.transition`.** Every animation belongs to one row's own frame. An animated diff inside a lazy stack is the pairing `docs/chat-list-hang.md` warns about, and a transition would also play an entrance for a row the stack realizes mid-scroll. A removed piece cuts rather than shrinking: nothing is left in the hierarchy to animate.
- **Each block of the turn in flight types itself out**, in `RevealedMarkdownBlock`. Every block is an ordinary `.markdown` piece keyed by its block index and carrying its own source, so the piece keeps its identity when the block completes and the reveal carries on instead of the text snapping to full. Before this, the arriving block was a `.streaming` piece keyed `stream/live`, and the identity change at each paragraph boundary skipped two animations at once: the end of the finished block and the start of the new one.
- **A completed block finishes at half pace.** `RevealProgress.finish` reveals what the block still owes over half the duration the same characters would have taken mid-stream, so the tail reads as catching up rather than as more typing.
- **`RevealClock` keeps the two in order.** A block finishing and the next block opening land on the same stream tick in two different views, so without it they would type at once. The finishing block claims its duration; the opening block delays by whatever is left, capped at one full reveal so an interrupted reveal cannot stall the block behind it. One clock per list, which is one per tab, passed through the environment and never read from a `body`.
...
- Two `AppSettings` toggles in the Settings window, defaulting on and independent of each other: `animateChatMotion` covers heights, arrivals and the composer's room together, and `animateCharacterReveal` covers the reveal. `animateChatMotion` keeps its original `animateRowHeight` defaults key, so an existing preference still reads.
...
- **Reading the directory needs no special handling for `EnterWorktree`.** Its `tool_use` input records the absolute path, but every transcript line afterwards also carries the new `cwd`, verified on a real session that moved into `.worktrees/…` mid-run. So reading `cwd` from the newest transcript line picks up `EnterWorktree` and every other directory change through one mechanism. `SessionJSONLReader` already reads these files.
- **Finding the file does need special handling.** `EnterWorktree` keeps the session id but Claude Code *moves* the transcript into a project directory keyed by the worktree's path, so the path derived from the task's original folder stops being written to. `TranscriptStore` searches the projects tree by session id when a watched file reads as empty, and re-points the tab. See [Worktrees](#worktrees).
...
- [x] A conversation minimap, like Sublime Text's, with bookmarks to navigate by — so a long transcript can be skimmed and most of the agent's output skipped.
...
**What shipped.** A panel down the chat's right edge, taking real layout width rather than floating. The user's own prompts are the legible content — one truncated line each, whatever their length — and every run of agent output between two of them collapses into one de-emphasized area, weighted by a log-compressed character estimate. Clicking an entry scrolls that piece to the top of the viewport. The map follows the reader through `onScrollTargetVisibilityChange`, and scrolls itself from its *own* weight space rather than the chat's offset, because the chat's rows are wildly uneven and its offset moves in jumps that would make the map twitch. `ChatOutline` is a pure function over `[ChatPiece]` and carries the tests; `ChatMinimap` is judged by eye.

Bookmarks did not ship. The prompts turned out to be the bookmarks — there are few enough of them, and they are what the reader anchors on — so a second, user-placed marker is worth re-asking about before building.

**Animated `scrollTo` into the list is fine, and the earlier caution here was wrong.** The controlled trials in `docs/chat-list-hang.md` falsified animation as a participant: the hang reproduced with the animation removed, and again with every row a blank rectangle. The loop is inside `LazyVStack`'s own height estimation, triggered by height contrast between realized rows. What stays banned is feeding a *measured* layout height back into the piece model.

**The anchors** come from `ChatPiece`'s `messageID` and `role`, which the outline groups by. A prompt long enough to split across pieces stays one landmark, and a piece's `id` is the scroll target, so no block-level field was needed after all. Telling the user's real prose from everything else recorded under their role — interruption markers, slash-command caveats and output, task notifications — is `InjectedContent.isUserProse`, which already drew that line for the chat.
...
- **The transcript file moves with the session.** `EnterWorktree` keeps the session id but Claude Code relocates the `.jsonl` into a project directory keyed by the worktree's path, so the path Plume derived from the task's original folder is never written to again. Worse, `FileWatcher` recreates the file it watches, so the move leaves a zero-byte stub behind and the tab reads it as an empty conversation. `TranscriptStore` treats an empty read as the signal to call `SessionJSONLReader.locateTranscript(sessionID:)` — a stat per project directory — and re-points the tab at wherever the session went. A session that has genuinely written nothing finds nothing and keeps waiting.
