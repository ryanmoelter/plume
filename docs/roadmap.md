...
- [ ] Make the statusline capture installable with confidence — today it breaks the statusline in every terminal outside Plume.
...
- **The statusline install is not safe to recommend yet**, though the pieces are all there. `StatuslineInstaller.install()` backs up the existing `statusLine`, writes a generated capture script, and points `~/.claude/settings.json` at it; the script writes the payload to `$PLUME_EVENTS_DIR/$PLUME_TASK_ID/$PLUME_TAB_ID.statusline.json` and then chains to the previous command. Three problems, in order of severity. **First, `settings.json` is global but the script assumes Plume.** It runs `set -u` and dereferences the three `PLUME_*` variables, which only exist in a terminal Plume launched — so in any other terminal it aborts with `PLUME_EVENTS_DIR: unbound variable` before reaching the chain, and the user's real statusline vanishes everywhere but Plume. Guarding those variables and skipping straight to the chain when they're absent is the fix, and it's small. **Second, Debug and Release install different paths.** `AppPaths.directoryName` resolves to `Plume.debug` under the debug bundle ID, so installing from a debug build writes a `statusLine` pointing into `Plume.debug/hooks/`, and a later release build neither uses nor recognizes it — `isAlreadyInstalled` compares against its own path, so the Settings UI will offer to install over it and back up Plume's own script as the chain target. **Third, the enable toggle does nothing.** `AppSettings.statuslineCaptureEnabled` is written by the toggle and by install/restore, and read nowhere else in the app; capture is on whenever the script is installed, so the toggle looks like a switch and isn't one. Worth deciding whether it drives anything or goes away.
...
- [x] Bundle Newsreader and make it the chat's prose font.
