...
4. The `isDetached` writes: one threshold, no hysteresis. Addressed structurally in **What landed**, not falsified.
...
- **Prose and list items take a piece each**, unconditionally: one per paragraph of a quote or a markdown paragraph, one per list item. The seam is a gap the source already has, so it costs the reader nothing, and waiting for a height to justify it left the spread uneven for no gain. A quote's bar is drawn through the gap on every piece after the first — `MarkdownBlock.quote` carries `continues` — so a split quote still reads as one. The estimator's error comes from the spread within the realized set rather than from how many items there are, which is what Trial E3 found when a wall of small uniform rows proved harmless. On the CAYN planning thread this took the tallest quote from 576 pt to 288 pt, the median from 384 pt to 72 pt, and left nothing over the ceiling.
...
- **The streaming overlay splits too.** Every block of the turn in flight is an ordinary `.markdown` piece keyed by its block index, carrying its own source and a flag for whether the stream is still writing it. The block still arriving is never split, however long it grows; the settled ones split like any other. Parsing is not prefix-stable, so a settled piece can be reinterpreted mid-stream; the remount is accepted.
- **The arriving block is keyed by its index, not by being the live one.** A `stream/live` id meant the piece drawing a block changed identity the moment that block completed, which threw away the reveal's progress: the last characters of the finished block were never typed, and the block below it appeared whole. Keying both states the same way keeps one view across the handoff, so the reveal finishes what it owes.
...
- Piece ids are deterministic from the message id and the block's original index, so a re-parse or a tool result landing keeps a row's expanded state alive. They are also unique across kinds, by the **depth** of the `/`-separated path rather than by naming the kind: one component past the message id for a whole block, two for a markdown block inside it, three for a segment of a split one. Every component is an `Int`, so no shorter path can collide with a longer one. cmux found that a duplicate `ForEach` id thrashes lazy layout into this same freeze on its own, and the failure mode is a hang rather than a visible glitch, so `ChatPieceSplitterTests` asserts uniqueness over a transcript covering every kind.
- **Short pieces are not packed back into larger items**, and should not be. A packer cannot honour both a floor and a ceiling — a 26 pt piece before a 588 pt one has to violate one of them — and an item keyed by its first piece loses the `@State` of every piece that moves to a different parent when the packing shifts. The ceiling does the work on its own; if the small side ever turns out to matter, the answer is a real minimum height on the shortest rows (Trial E1), not packing. A list packed up to twelve items per piece until Sep 11 for the same reason a packer is tempting — fewer items — and that is the reasoning the item count does not support.

Plume also has a second chat list container now, `docs/chat-list.md`, which lays out the same pieces itself instead of asking `LazyVStack` to estimate them. It has no estimator and so none of the loop this document tracks. The piece split and the 300 pt ceiling above still apply to it, because both engines draw from the same `ChatPieceSplitter` output.

### Hypothesis 4, the jump-back flag

Addressed by construction rather than falsified, so there is nothing left to observe. cmux hit a hang whose whole cause was a scroll-to-bottom pill: a threshold-derived flag, written from a visibility callback, **read by the body that builds the lazy stack**. Plume's `isDetached` had the same shape, and Trial 0 recorded the matching symptom — the list body's re-runs "were all `_isDetached`". That is not the engine of the sustained spin Trial 0 captured, which stayed hot after the body went quiet, but it is a second route into the same freeze and the piece split does nothing about it.

Two halves, both now closed:

- **The threshold has hysteresis.** `ChatScrollAnchor.isDetached(distanceFromBottom:wasDetached:)` latches: out past `detachThreshold` (320 pt) it turns on, back inside `reattachThreshold` (200 pt) it turns off, and between the two it holds. A distance resting near one value can no longer flip the flag every frame. The cost is that the button's visibility inside the band depends on which way the reader came, which is imperceptible and deliberate.
- **Nothing the lazy stack's body reads is derived from the scroll position.** The flag now lives in an observable box that only `ChatJumpToBottomButton` reads, so flipping it re-renders the button and not the list; the latch behind it lives in `ScrollFollowState`, which is a plain class precisely so per-frame writes are not view changes. `.defaultScrollAnchor(_, for: .sizeChanges)` keys off `jumpTargetID` instead — that anchor was unconditional until `25caa7b`, which made it conditional to stop a *jump* landing at the bottom edge instead of the top, and a jump is what the condition should have named. `jumpTargetID` is already read by the `ForEach`, and only ever moves id → nil, so it cannot chatter.

Unverified against the original trigger, the same way the Sep 4 fix was: the hang needs a 27" display and real trackpad momentum, and the probes that would show the flag going quiet (`PLUME_CHAT_DIAG`, `capture-chat-hang.sh`, the `ryanm/chat-hang-diagnosis` branch) no longer exist. `detect-chat-hang.sh` is what survives, and it detects a hang rather than attributing one.
...
**Heights the ceiling does not bound**, so verification knows where to look: a table; a single paragraph over the ceiling, which has no blank line to split at; `ChatImageView` up to 320 pt; and the live tail of the streaming overlay, which is one item until the next block starts.

A disclosed body is no longer among them. `ThinkingRow` and `InjectedContentRow` bound theirs at `ChatPieceMetrics.maxDisclosedHeight` and scroll inside it, the way `ToolCallRow`'s sections already did — an injected line carries a whole approved plan, and expanding one handed the stack an item many times its neighbours' height.
...
## Sep 11: it came back on a planning thread

Reproduced by hand on the CAYN planning transcript (a Notability thread, 2.0 MB, 663 lines), seeded through `PLUME_SEED_TRANSCRIPT_PATH` and hung within seconds of scrolling. Two `sample`s ten seconds apart agree, so it was a sustained loop rather than a finishing burst, and the signature is the one this document already describes: `LazySubviewPlacements`, `LazyLayoutViewCache`, `measureEstimates`, `propagate_dirty`, `signalPrefetch`, and `RootGeometry.value.getter` re-measuring from the root. The hidden terminal tabs are in the pass, as before.

**No Plume code appears in the loop**, and in particular none of the jump-back flag's machinery — `isDetached`, `ChatScrollAnchor`, `defaultScrollAnchor`, `JumpButtonVisibility` are all absent from both samples. So the hypothesis-4 work above is unfalsified rather than proven: the loop it targets did not run here.

What the thread had was height contrast the ceiling was not catching. Its tool results span 42 to 28,621 characters, and three kinds of piece were unbounded: an expanded `ThinkingRow` or `InjectedContentRow`, and prose. The quotes were the ones that mattered — 27 of them, median 384 pt, tallest 576 pt, 24 over the ceiling — because `ChatPieceSplitter` left prose whole on the reading that it "is never long enough to be worth" splitting. A planning thread's quotes are.

Fixed by the three entries in **What landed**: the disclosed bodies bound, and prose and list items split one per piece. The same measurement after: 104 quote pieces, median 72 pt, tallest 288 pt, none over the ceiling. Scrolling the thread no longer hangs.

Two things this cost, worth knowing:

- **The probes are gone.** `PLUME_CHAT_DIAG`, `ChatHangDiagnostics`, `ChatListBisect` and `capture-chat-hang.sh` are absent from the tree, and `ryanm/chat-hang-diagnosis` is on no reachable remote. Only `detect-chat-hang.sh` survives, so this reproduction was read from raw samples. The **"Chat Item Boxes"** toggle in the sidebar footer (DEBUG only, `ChatItemOutlines`) outlines every lazy item with its kind and measured height, which is the cheap way to see a spread without rebuilding the probes.
- **`wrappedLines` double-counted.** It added explicit newlines *and* `characters / 60`, as though the text were one unbroken run, so a 13-line 949-character quote estimated at 28 lines. Reading a piece spread through it overstated every height by roughly twice. It is deleted along with the list-packing helpers that used it; nothing estimates prose height any more, because nothing needs to.
