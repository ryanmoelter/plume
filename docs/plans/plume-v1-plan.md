...
- **2026-08-31 — WP4.1's resume overlay replaced by lazy auto-resume.** `AgentResumeOverlayView` (Resume/Start Fresh buttons) is deleted. `claude --resume <id>` costs nothing until a message is sent — it just reads the transcript and waits at the prompt — so gating it behind a button added friction without protecting anything. Meanwhile "Start Fresh" was the actually destructive, unconfirmed action: it discards Plume's only pointer back to the conversation. An agent tab with a stored `agentSessionID` now resumes automatically the first time it becomes the *selected* tab (not at mount, since `TabContentView` keeps every tab mounted; not at app launch, to avoid spawning `claude` for every agent tab on relaunch), exactly once per tab per run (`AutoResumingAgentTabView`, decision logic in the pure `AgentAutoResume.shouldResume`). The missing-working-directory guard carries over unchanged: resume is withheld and `AgentMissingDirectoryView` shown instead. "Start Fresh" moved to a deliberate, confirmed action — tab context menu and the Tab menu (`PlumeCommands`) — with a `.confirmationDialog` before clearing `agentSessionID`. Runtime-verified via the `PLUME_SEED_*` recipe (added `PLUME_SEED_AGENT_SESSION_ID`): selecting other tabs first does not spawn `claude`; selecting the agent tab spawns `claude --resume` exactly once; a missing working directory suppresses it entirely.

- **2026-08-31 — Post-v1: derived tab titles and sidebar detail lines.** Additions past the v1 scope, to make the sidebar readable enough to replace cmux.
  - **Titles.** Terminal tabs already had a title from libghostty, but nothing consumed it — and nothing could have: `TerminalViewState` is a Combine `ObservableObject`, and `TerminalSession` holds it `@ObservationIgnored`, so reading `state.title` from a SwiftUI view renders once and never updates. `TerminalSession` now mirrors `title`/`workingDirectory` into its own `@Observable` stored properties via Combine sinks, keeping the Combine detail inside `Plume/Ghostty/`. Agent tabs take Claude's own title from the transcript: Claude Code writes `{"type":"ai-title","aiTitle":"…"}` lines and rewrites them as a session develops, so the **last** one is current. `SessionJSONLReader.latestAITitle` scans in reverse (cheap on the 2300-line transcripts observed) and `AgentTitleMonitor` watches each tab's transcript, debounced at 1s because transcripts are written constantly but retitled rarely. `TitleStore` holds the derived title per tab in memory, snapshotting to `TaskTab.title` so a relaunch has a label before anything reconnects.
  - **Which tab names a task.** A new `WorkTask.lastFocusedAgentTabID` tracks the last *agent* tab focused, so dipping into a terminal tab does not relabel the sidebar row. Resolution order is last-focused agent tab → selected tab → first tab. All selection now routes through `TaskStore.selectTab`, the single place the two fields can be kept in step.
  - **Naming.** ⌘N no longer opens the rename editor, and `createTask`'s title defaults to `""` instead of `"New Task"`; a task shows the agent's title until the user deliberately renames it. Clearing a name reverts to the agent's title rather than restoring a placeholder.
  - **Ports: attempted, then dropped.** A `PortMonitor` polling `ps` + `lsof` and attributing listeners by ppid chain was built and unit-tested, but removed before landing. The blocker was `TerminalView.foregroundPid`: with two live PTYs confirmed on their own ttys, every sweep still logged `0 task(s) with live surface pids`, so no probe ever ran. The timer, the activity suspend/resume, and the parsing were all verified working — only the pid lookup failed. Ryan called it not worth the cost. Anyone reviving this should start there, and treat `ttyName` as the more promising correlation key, since the tty is what the process tree actually agrees on.
  - **Sidebar row.** Status, branch, and directory each get their own dim line under the name, omitted entirely when absent. `TaskRowDetails` builds them as an ordered `[String]`, so collapsing to one joined line later is a different join, not a rewrite.
...
- **[x] WP4.2 Polish** *(done 2026-08-31)*: menu commands, shortcuts (⌘1–9, ⌘⇧]/[), empty states, Settings stub (worktree base path override, provider field), archive view, app icon. *Accept:* keyboard-only workflow; first launch self-explanatory.
  - Menu commands were largely already done in WP2.3; the gaps found by audit were an "Archive Task" command (previously only reachable via right-click context menu — added ⌘⌃A) and a way to reach the archive at all (added "Show Archive…" at ⌘⇧A). No other churn to what already worked.
  - Empty states: `TabContentView` had no fallback when a task's last tab is closed (`ZStack` over zero tabs renders blank) — added a `ContentUnavailableView`. Sidebar's no-tasks and detail's no-selection states already existed from earlier WPs.
  - Settings: `AppSettings` (`@MainActor @Observable`, `UserDefaults`-backed) with `worktreeBasePath: String?` and `providerID: String`, exposed via a `Settings` scene (`Plume/UI/Settings/SettingsView.swift`, automatic ⌘,). Both fields are real seams, not decorative: `WorkspaceProvisioner.worktreePath`/`createWorktree` take an optional `basePath` that `NewWorktreeSheet` reads from `AppSettings.shared`, and `AgentLauncher` resolves the provider through a new `AgentProviderRegistry` keyed on `AppSettings.shared.providerID` (falls back to Claude Code on an unrecognized ID, logged, since it's the only provider implemented in v1).
  - Archive view: `Plume/UI/Sidebar/ArchiveView.swift` lists `isArchived` tasks with Unarchive/Delete, presented as a sheet from `MainWindow`, reachable from the sidebar toolbar and the new menu command.
  - App icon: generated programmatically (Python/PIL, not a placeholder) — a fanned stack of terminal-card panes on an indigo-to-violet gradient, evoking parallel agent tabs. All 10 required mac idiom sizes written into `AppIcon.appiconset` with an updated `Contents.json`.
  - New tests: `AppSettingsTests` (default values, both fields persist across a fresh instance over the same `UserDefaults` suite), `AgentProviderRegistryTests` (known ID resolves, unknown ID falls back, settings path forwarded), and three `WorkspaceProvisionerTests`/`WorktreeProvisioningTests` cases for the `basePath` override (path computation, empty-string fallback, and an end-to-end `createWorktree` against a real repo confirming the in-repo `.gitignore` bootstrap is skipped when overridden).
  - Verified: clean build, full `PlumeTests` pass (all suites, including the new ones). Runtime verification (app launches without crashing, no idle/orphaned processes after quit, app icon reaches the built bundle) was delegated to a runtime-check agent using the process-tree/unified-log recipe.
  - **Left unverified** — no Accessibility permission in this environment: actually clicking through the Settings window, exercising ⌘1–9 / ⌘⇧]/[ / ⌘⇧A / ⌘⌃A by real keypress, and eyeballing the app icon and empty-state layouts. These are asserted by code review and unit coverage of the underlying logic, not by driving the UI.
  - This completes Phase 4 and the v1 plan — every WP in every phase is now checked off.
