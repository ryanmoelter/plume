...
- **Live streaming.** `ChatStreamHandoff` and `StreamingBlocks` show the assistant's reply as it's written, then hand off to the transcript's own trailing message once it catches up, without a visible gap or duplicate.
...
- **Quota and cost pushed as events.** `rate_limit_event` and each turn's `total_cost_usd` reach `HeadlessSession` directly. This retired the statusline capture, a script Plume used to install into `~/.claude/settings.json` to scrape the same numbers out of band. `StatuslineUninstall` removes that install once, at launch, for anyone who has it, without touching a statusline the user configures afterward.
