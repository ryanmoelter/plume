...
Both are signed with Developer ID and notarized. That used to be the shared build's distinction, but the sleep helper changed it: `SMAppService` registers a LaunchDaemon only out of a notarized app, so a Development-signed local install would have a lid-closed toggle that never works. The **One-time setup** under *Sharing a build* is a prerequisite for the local install too.
...
**The override is volatile by design.** The helper writes `SleepDisabled` to the live registry with `IORegistryEntrySetCFProperty`, never through `pmset`, so it clears on reboot and never touches `/Library/Preferences/com.apple.PowerManagement.plist`. On top of that the helper clears it on launch, when the app's XPC connection drops, and when 90 seconds pass without a heartbeat; the app clears it on termination and whenever the plain assertion is released. The worst case after any crash is "awake until reboot", never "never sleeps again".
